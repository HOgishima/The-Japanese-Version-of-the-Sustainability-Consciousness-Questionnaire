...
modeltxt <- paste(
    'SC =~ K + A + B',
    'K   =~ K_ENV_P + K_SOC_P + K_ECO_P',
    'A   =~ A_ENV_P + A_SOC_P + A_ECO_P',
...
layered_three_factor <- fitMeasures(fit,c('RMSEA','CFI','TLI'))
...
table_layered_three_factor_p <- apa_table(
  table_list
  , format = 'latex'
  , align = c("l", rep("c", 3))
  , digits = 3
  , caption = 'Factor loading of each item for Three-factor hierarchical parceled model.'
  , note = 'P in each factor item indicates that the corresponding data were parceled. 
  \\textit{SE} = Standard Error, \\textit{P} = P value. 
    K = knowingness; A = attitudes; B = behaviour; ECO = economic; SOC = social; ENV = environmental; SC = sustainability consciousness.'
