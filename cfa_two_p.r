...
cfa_two_p <- fitMeasures(fit,c('RMSEA','CFI','TLI'))
...
table_cfa_two_p <- apa_table(
  table_list
  , format = 'latex'
  , align = c("l", rep("c", 3))
  , digits = 3
  , caption = 'Factor loading of each item for Two-factor parceled model.'
  , note = 'P in each factor item indicates that the corresponding data were parceled. 
  \\textit{SE} = Standard Error, \\textit{P} = P value. 
    KA = knowingness/attitudes; B = behaviour; ECO = economic; SOC = social; ENV = environmental; Sus Cons = sustainability consciousness.'
