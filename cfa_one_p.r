...
# 1ˆöŽq
modeltxt <- paste(
    'SC   =~ K_ENV_P + K_SOC_P + K_ECO_P + A_ENV_P + A_SOC_P + A_ECO_P + B_ENV_P + B_SOC_P + B_ECO_P',
...
cfa_one_p <- fitMeasures(fit,c('RMSEA','CFI','TLI'))
...
table_cfa_one_p <- apa_table(
  table_list
  , format = 'latex'
  , align = c("l", rep("c", 3))
  , digits = 3
  , caption = 'Factor loading of each item for One-factor parceled model.'
  , note = 'P in each factor item indicates that the corresponding data were parceled. 
    \\textit{SE} = Standard Error, \\textit{P} = P value. 
    K = knowingness; A = attitudes; B = behaviour; ECO = economic; SOC = social; ENV = environmental; Sus Cons = sustainability consciousness.'
